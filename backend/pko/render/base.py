...
# Класс `authorship`, а не `origin`: `origin` уже занят бейджем происхождения
# значения поля (VERIFIED/OBSERVED/…), и переиспользование сломало бы его вид.
ORIGIN_LLM = ("Этот абзац сформулировала языковая модель по готовой модели PKO. "
              "Факты, проверки и решение вычислены детерминированно.")
ORIGIN_TEMPLATE = "Этот абзац собран по модели детерминированно, без языковой модели."


def authorship(source: str) -> str:
    """Строка авторства обзора по источнику текста писателя."""
    return ORIGIN_LLM if source == "llm" else ORIGIN_TEMPLATE
