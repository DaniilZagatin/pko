...
Роли:
  * `matcher`  — единый агент: текст слайдов PPTX + инструменты поиска по коду
                 → пункты плана и вердикты по ним, за одну сессию;
...
общий дефолт для «вернуть текст/JSON по структурированному входу», чтобы обе
роли работали сразу на одной уже настроенной паре endpoint/ключ).
`PKO_MATCHER_*`/`PKO_REPORTER_*` переопределяют его по отдельности, если
какой-то роли нужна другая модель. В отличие от `matcher`, `reporter`
необязателен: без него отчёт собирается как обычно, просто без сводного
вывода.
