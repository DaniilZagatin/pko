...
class LlmContractTest(unittest.TestCase):
    """`--llm` — заявка на текст модели, а не пожелание."""

    def test_llm_without_writer_endpoint_stops_before_analysis(self):
        """Пройти анализ и выдать шаблон значит подменить отчёт молча."""
        import os
        from unittest.mock import patch

        from pko.cli import cmd_analyze, _build_parser

        args = _build_parser().parse_args(
            ["analyze", "--repo-path", str(ensure_fixture()), "--llm", "--out", "/tmp/pko-x"])
        env = {k: v for k, v in os.environ.items()
               if not any(m in k for m in ("WRITER", "DEEPSEEK", "GLM", "LLM_", "PKO_"))}
        with patch.dict(os.environ, env, clear=True):
            with self.assertRaises(PkoError) as ctx:
                cmd_analyze(args)
        self.assertIn("писателя не настроен", ctx.exception.message)
        self.assertIn("--llm", ctx.exception.hint)
