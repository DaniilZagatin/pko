...
def _refs(facts, limit: int = 3) -> list[str]:
    """Ссылки на факты для карточки: место плюс то, что там увидели.

    Голое `path:line` в колонке «Ссылка на факт» требовало открыть репозиторий,
    чтобы понять, о чём речь. Основание факта уже записано рядом — показываем
    его, а координату оставляем для сверки.
    """
    out = []
    for fact in facts[:limit]:
        where = f"{fact.path}:{fact.line}" if fact.line else fact.path
        basis = (fact.basis or "").strip()
        out.append(f"{where} — {basis}" if basis else where)
    return out
...
def _trajectory_recoverable(ctx: GateContext):
    # Шагом траектории может быть узел графа, команда CLI или обработчик
    # очереди — вопрос в том, восстановима ли последовательность, а не в том,
    # каким фреймворком она написана.
    nodes = policies.steps(ctx.extraction)
    routes = policies.entrypoints(ctx.extraction)
    if nodes:
        return (
            PASS,
            f"траектория восстановлена: узлов графа {len(nodes)}, "
            f"переходов {len(policies.transitions(ctx.extraction))}",
            _refs(nodes),
        )
    if routes:
        return (
...
def _read_only_proven(ctx: GateContext):
    writes = policies.data_writes(ctx.extraction)
    reads = policies.data_reads(ctx.extraction)
    ...
    if writes:
        ...
        # Ссылки показываем сначала на исполнение: если изменение есть и вне
        # миграций, читателю важно увидеть именно его.
        shown = (runtime or migrations)[:3]
        return (
            FAIL,
            f"найден SQL, изменяющий данные ({len(writes)} мест"
            + (f", {note}" if note else "")
            + ") — инвариант «только чтение» нарушен",
            _refs(shown),
        )
...
def _limits_present(ctx: GateContext):
    limits = policies.controls(ctx.extraction)
    if not limits:
...
def _coverage_sufficient(ctx: GateContext):
    ...
    # Знаменатель — периметр первой версии, а не всё дерево: фронтенд PKO не
    # разбирает, и держать его в проверке значило бы объявлять границу
    # инструмента недостатком анализируемой системы.
    if ratio >= 0.6:
        return PASS, f"проанализировано {ratio:.0%} файлов backend-периметра", [ref]
    return (
        FAIL,
        f"проанализировано только {ratio:.0%} файлов backend-периметра — "
        f"вывод не покрывает его целиком",
        [ref],
    )
