...
# Что в `PkoObject.links` является ссылкой на другой объект модели. Остальные
# записи — техническая привязка: `package` (каталог реализации), `limit_key` и
# `stable_key` (идентичность, не зависящая от позиции в списке). Определение живёт
# здесь, потому что смысл связей задаёт модель: и валидатор, и отчёты обязаны
# понимать их одинаково, иначе технический ключ протекает в раздел «Связи» и
# выглядит как ссылка на несуществующий объект.
...
class Evidence:
    ...
    def ref(self) -> str:
        """Полная ссылка для машин и для сверки: `путь:строка@коммит`."""
        loc = f"{self.path}:{self.line}" if self.line else self.path
        return f"{loc}@{self.commit[:8]}"

    def where(self) -> str:
        """Место без коммита: он один на всю версию и печатается в шапке.

        `path:line@6a225969` в каждой строке паспорта — это шум: хеш
        повторяется дословно сотни раз и ничего не добавляет читателю.
        """
        return f"{self.path}:{self.line}" if self.line else self.path

    def explain(self) -> str:
        """Чем это доказано, словами. Пусто — значит, объяснения нет."""
        return self.basis.strip()
