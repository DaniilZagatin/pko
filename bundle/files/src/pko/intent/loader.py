...
# Как называется внешний intent в доказательствах. Абсолютный путь сюда не
# годится: он попадал в Gate Card и в паспорта вместе с домашним каталогом и
# корпоративным логином оператора, а читателю отчёта он всё равно бесполезен —
...
def load_intent(tree, commit: str, override_path: str | Path | None = None) -> IntentResult:
    ...
    if override_path:
        ...
        return _parse(text, external_source(p, text), commit)
