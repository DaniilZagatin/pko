...
# Пак → имена файлов, за которыми стоит объявленный контракт. Расширение здесь
# не годится: спецификация и обычная конфигурация — оба `.yaml`.
_PACK_BY_FILENAME = {
    "contracts": ("openapi", "swagger", "asyncapi", "api-spec", "api_spec",
                  "tools.json", "functions.json", "manifest.json"),
}
...
def detect(tree: Tree, extraction: Extraction | None = None) -> StackProfile:
    ...
    for pack, needles in _PACK_BY_FILENAME.items():
        hits = sorted({
            path.rsplit("/", 1)[-1] for path in files
            if any(needle in path.rsplit("/", 1)[-1].lower() for needle in needles)
        })
        if hits:
            packs.setdefault(pack, []).extend(f"файл {h}" for h in hits[:3])
