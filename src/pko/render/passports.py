...
from pko.render.base import authorship, esc, field_html, gaps_section, meta_bar, page, tag
...
def render_passports(
    ...
    body = (
        meta_bar(model)
        + f'<div id="cardsView">{_overview_section(overview, overview_source)}{"".join(cards)}'
        + gaps_section(model, "!")
        + "</div>"
        + _script(data)
    )
...
def _overview_section(text: str, source: str = "") -> str:
    """Обзор перед картотекой: читатель начинает с картины, а не с карточек.

    Авторство помечено: паспорта ниже восстановлены из кода, а этот абзац мог
    написать писатель, и смешивать их читателю не следует.
    """
    if not text:
        return ""
  ...
  <div class="section">
    ...
    <div class="section-body"><div class="lead">{esc(text)}</div>
