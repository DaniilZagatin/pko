...
from pko.render.base import authorship, compact_value, esc, gaps_section, meta_bar, page, tag
...
def render_taxonomy(model: PkoModel, summary: str = "", summary_source: str = "") -> str:
    body = [meta_bar(model)]
    if summary:
        body.append(_summary_block(summary, summary_source))
...
def _summary_block(summary: str, source: str = "") -> str:
    """Обзор с указанием авторства: объяснение модели не должно весить как факт."""
    origin = authorship(source)
    return f"""
  <div class="section">
    ...
    <div class="section-body"><div style="font-size:0.92rem;">{esc(summary)}</div>
      <div class="authorship">{esc(origin)}</div></div>
  </div>
"""
