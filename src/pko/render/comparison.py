...
def render_comparison(diff: ModelDiff, narrative: str = "") -> str:
    ...
    for kind, title in _KINDS:
        ...
        for obj in items:
            changes = "".join(
                f"<div><strong>{esc(c.label)}:</strong> "
                f"<span class='before'>{esc(_short(c.before))}</span> → {esc(_short(c.after))}</div>"
                for c in obj.changes[:6]
            )
            more = (f"<div class='before'>…ещё изменений: {len(obj.changes) - 6}</div>"
                    if len(obj.changes) > 6 else "")
            rows.append(
                f"<tr><td class='id-cell'>{esc(obj.id)}</td><td>{esc(obj.name)}</td>"
                f"<td><span class='status {_STATUS_CLASS[obj.status]}'>"
                f"{_STATUS_TEXT[obj.status]}</span></td>"
                f"<td>{changes}{more}</td></tr>"
            )
        body.append(
            f"""
  <div class="section">
    ...
    <div class="section-body">
      <table class="cmp"><thead><tr><th>ID</th><th>Название</th><th>Статус</th>
      <th>Что изменилось</th></tr></thead><tbody>{''.join(rows)}</tbody></table>
    </div>
  </div>
"""
        )
...
def _counts_section(diff: ModelDiff) -> str:
    ...
    for kind, title in _KINDS:
        ...
        rows.append(
            f"<tr><td>{esc(title)}</td><td>{before}</td><td>{after}</td>"
            f"<td><span class='status {cls}'>{esc(mark)}</span></td></tr>"
        )
    return f"""
  <div class="section">
    <div class="section-header">
    ...
    <div class="section-body">
      <table class="cmp"><thead><tr><th>Тип</th><th>{esc(diff.left_label)}</th>
      <th>{esc(diff.right_label)}</th><th>Diff</th></tr></thead>
