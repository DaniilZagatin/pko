...
class IntentOverrideFailureTest(unittest.TestCase):
    ...
    def test_cli_turns_the_refusal_into_exit_code_one(self):
        """Стек в консоли не отличить от падения PKO; ошибка аргумента — это код 1."""
        import contextlib
        import io

        from pko.cli import main

        target = self.tmp / "intent_dir_cli"
        target.mkdir()
        stderr = io.StringIO()
        with contextlib.redirect_stderr(stderr), contextlib.redirect_stdout(io.StringIO()):
            code = main(["gate", "--repo-path", str(ensure_fixture()), "--branch", "master",
                         "--out", str(self.tmp / "out"), "--intent", str(target)])
        self.assertEqual(code, 1)
        self.assertIn("не на файл", stderr.getvalue())
