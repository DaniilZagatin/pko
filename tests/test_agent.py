...
class PromptTest(unittest.TestCase):
    ...
    def test_every_pack_is_loadable_and_named_in_the_detector(self):
        ...
        from pko.agent.stack import (
            _PACK_BY_DEP, _PACK_BY_EXT, _PACK_BY_FILENAME, _PACK_BY_MECHANISM,
        )

        known = (set(_PACK_BY_DEP) | set(_PACK_BY_EXT) | set(_PACK_BY_FILENAME)
                 | set(_PACK_BY_MECHANISM.values()))
    ...
    def test_contracts_pack_activates_on_a_specification_file(self):
        """Спецификация опознаётся по имени файла: расширение у неё общее с конфигурацией."""
        from pko.agent.stack import detect

        class _Tree:
            files = ["docs/openapi.yaml", "src/app.py"]

            def read(self, path):
                return ""

        self.assertIn("contracts", detect(_Tree()).packs)
